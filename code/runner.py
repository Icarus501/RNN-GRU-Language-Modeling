...
class Runner(object):
    ...
    def __init__(self, model: Model):
        self.model = model  # all puppies are dogs, not all dogs are puppies.
    ...
    def compute_loss(self, x, d):
        ...
        loss = 0.
        ##########################
        # r = RNN(self.model.vocab_size, self.model.hidden_dims, self.model.out_vocab_size)
    ...
    def compute_loss_np(self, x, d):
        ...
        ##########################
        y, s = self.model.predict(x)
        encoded_d = make_onehot(d[-1], self.model.vocab_size)

        # for t in range(len(x)):
        loss -= np.matmul(encoded_d, np.log(y[len(x) - 1]))
    ...
    def compute_mean_loss(self, X, D):
        ...
        # y, s = self.model.predict(x) # all puppies are dogs, not all dogs are puppies.
        #
        # for t in range(len(x)):
        #     encoded_d = make_onehot(d[t], self.model.vocab_size)
        #     loss -= np.matmul(encoded_d, np.log(y[t]))
        ...
        total_loss = 0
        for t in range(len(X)):
            total_loss += self.compute_loss(X[t], D[t])
        mean_loss = total_loss / len([words for x in X for words in x])
        ##########################
    ...
    def train_np(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100,
        ...
        for epoch in range(epochs):
            ...
            if log:
                ...
                # stdout.write(f" norms of the gradients: deltaW:{np.linalg.norm(self.model.deltaW)}")
                stdout.write("")
                stdout.flush()
...
if __name__ == "__main__":
    ...
    if mode == "train-lm-rnn":
        ...
        #train_size = 1000  # Q2(a)
        train_size = 25000  # Q2(b)
        dev_size = 1000  # first 1000 dev set
        vocab_size = 2000  # fixed
        test_size = 4000

        hdim = int(sys.argv[3])  # 25 or 50
        lookback = int(sys.argv[4])  # 0 or 2 or 5
        lr = float(sys.argv[5])  # 0.1, 0.5 or 0.05
        ...
        lm_rnn = Runner(rnn)
        best_mean_loss = lm_rnn.train(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, anneal=5,
                                      back_steps=lookback, batch_size=100, min_change=0.0001, log=True)

        adjust_mean_loss = adjust_loss(best_mean_loss, fraction_lost, q)

        print("the best mean loss on training set:", best_mean_loss)
        print("the best adjust_mean_loss on training set:", adjust_mean_loss)
        ...
        # report the mean loss on test set and perplexity
        run_loss = lm_rnn.compute_mean_loss(X_test, D_test)
        adjusted_loss = adjust_loss(run_loss, fraction_lost, q)

        print("the mean loss on test set:", run_loss)
    ...
    if mode == "train-np-rnn":
        ...
        mean_loss = rnn_np.train_np(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, anneal=5,
                                    back_steps=lookback, batch_size=100, min_change=0.0001, log=True)
        ...
        acc = sum([rnn_np.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(X_test)
    ...
    if mode == "train-np-gru":
        ...
        mean_loss = gru_np.train_np(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, anneal=5,
                                    back_steps=lookback, batch_size=100, min_change=0.0001, log=True)
        ...
        acc = sum([gru_np.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(X_test)
        print("Accuracy: %.03f" % acc)
